...
Renderer::Renderer(Window & parent) : OGLRenderer(parent) {
	...
	light = new PointLight(Vector3(-20.0f, 10.0f, -20.0f), Vector4(1, 1, 0, 1), 55.0f);
...
void Renderer::UpdateScene(float dt) {
	...
	direction ? scanX += 10 * dt : scanX -= 10 * dt;
	PointLight* s = dynamic_cast<PointLight*> (light);
	s->SetPosition(Vector3(scanX, 10.0f, -20.0f));
}
...
void Renderer::DrawShadowScene() {
	...
	PointLight* s = dynamic_cast<PointLight*> (light);
	viewMatrix = Matrix4::BuildViewMatrix(s->GetPosition(), Vector3(0, 0, 0));
